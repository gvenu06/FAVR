...
export async function fetchEpssScores(cveIds: string[]): Promise<Map<string, EpssScore>> {
  ...
  for (const batch of batches) {
    try {
      ...
      const data = response.data as EpssApiResponse
      if (data.data) {
...
/**
 * Simple fetch using Electron's net module (works even when Node fetch isn't available).
 */
function electronFetch(url: string): Promise<{ ok: boolean; status: number; data: any }> {
