 ...
 */
async function osvQuery(packageName: string, ecosystem: string, version: string): Promise<OsvVuln[]> {
  try {
    ...
    if (!response.ok) return [] // Don't fail the whole scan for one package

    const data = await response.json()
    return (data as any).vulns ?? []
  } catch {
